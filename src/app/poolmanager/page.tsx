  ...
  const handleAction = async () => {
    ...
    try {
      ...
      if (action === 'incentivize') {
        const tx = await poolManager.depositIntoIncentivePool(poolAddress, ethers.utils.parseUnits(amount, 18));
        await tx.wait();
        ...
        const tx = await poolManager.addLiquidity(tokenA.value, tokenB.value, ethers.utils.parseUnits(amountA, 18), ethers.utils.parseUnits(amountB, 18));
        await tx.wait();
        alert('Liquidity added successfully');
        console.log('Liquidity added');
      } else if (action === 'withdraw') {
        const tx = await poolManager.withdrawLiquidity(tokenA.value, tokenB.value, ethers.utils.parseUnits(amountA, 18), ethers.utils.parseUnits(amountB, 18));
        await tx.wait();
